                    ...
                    if (json.success) {
                        if (e.data.callback) {
                            if (typeof (e.data.callback) == 'function') {
                                e.data.callback(json.returnData);
                            }
                            //window[e.data.callback](json.returnData);
                        } else {
    ...
    var loadAndShowDialog = function (id, link, url) {
        ...
        $.get(url + separator + 'content=1')
            .done(function (content) {
                dialogs[id] = $('<div id="popupDialog" class="modal-popup">' + content + '</div>')
                    ...
                    .find('form') // Attach logic on forms
                        .submit({ callback: link.data('dialog-callback') }, formSubmitHandler)
                    .end();
            });
    };

    var initDialogs = function (links) {
